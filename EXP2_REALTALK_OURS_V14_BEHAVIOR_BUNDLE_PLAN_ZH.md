...
V14.9 完成 6/6、零 unresolved，并通过仅修改空白的排版归一化实现气泡数 6/6 一致；但问题
权限只有 4/6 一致。原因是冻结 V9 内容方向与新 Decision `question_plan` 同时进入 Actor：前者
可能包含“回问”，后者可能规定不问，形成两个互相冲突的权威来源。V14.10 不用正则删除自然
语言策略，而是把新 Decision 合同明确拆成：

- `content_focus`：只描述本轮贡献的内容，不允许携带提问指令；
- `question_plan`：是否提问以及提问类型；
- `question_target`：允许提问时唯一的信息槽，不提问时必须为空。
